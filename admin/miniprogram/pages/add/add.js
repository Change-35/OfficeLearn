  ...
  upload: function () {
    ...
    wx.chooseVideo({
      ...
      success(res) {
        ...
        let videos = res.thumbTempFilePath.split("/");
        wx.cloud.uploadFile({
          cloudPath: res.thumbTempFilePath, // 对象存储路径，根路径直接填文件名，文件夹例子 test/文件名，不要 / 开头
          filePath: `video/${videos[videos.length - 1]}`, // 微信本地文件，通过选择图片，聊天文件等接口获取
          success: res => {
